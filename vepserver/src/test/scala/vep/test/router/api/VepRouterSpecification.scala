...
import vep.test.controller.{VepControllersDBInMemoryComponent, VepControllersInMemoryComponent}

/**
 * @deprecated use [[vep.test.router.api.VepRouterDBInMemorySpecification]] instead
 */
@Deprecated
...
trait VepRouterDBInMemorySpecification extends Specs2RouteTest with VepApiRouter with VepControllersDBInMemoryComponent {
  override def actorRefFactory: ActorRefFactory = system
  lazy val validCredentialsUser = BasicHttpCredentials("abc@def.com", "abcd")
  lazy val validCredentialsAdmin = BasicHttpCredentials("admin@admin.com", "admin")
  lazy val invalidCredentials = BasicHttpCredentials("abc@def.com", "unknown")
